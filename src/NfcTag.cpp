  ...
  if (this != &rhs)
    {
      ...
      _ndefMessage = rhs._ndefMessage; // Shallow copy, ensure the ownership model works as expected.
    }
  return *this;
}
...
void NfcTag::print()
{
  Log.info("NFC Tag - %s", _tagType.c_str());
  Log.info("UID: %s", getUidString().c_str());
  if (_ndefMessage == nullptr)
    {
      Log.info("No NDEF Message");
    }
  else
    {
      _ndefMessage->print();
