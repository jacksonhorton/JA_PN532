...
uint32_t JA_PN532::getFirmwareVersion(void)
{
  ...
  if (_interface->writeCommand(pn532_packetbuffer, 1)) {
    return 0;
  }

  // read data packet
  int16_t status = _interface->readResponse(pn532_packetbuffer, sizeof(pn532_packetbuffer));
  if (0 > status) {
    return 0;
  }
...
uint32_t JA_PN532::readRegister(uint16_t reg)
{
  ...
  if (_interface->writeCommand(pn532_packetbuffer, 3)) {
    return 0;
  }
...
uint32_t JA_PN532::writeRegister(uint16_t reg, uint8_t val)
{
  ...
  if (_interface->writeCommand(pn532_packetbuffer, 4)) {
    return 0;
  }
...
bool JA_PN532::writeGPIO(uint8_t pinstate)
{
  ...
  return (0 < _interface->readResponse(pn532_packetbuffer, sizeof(pn532_packetbuffer)));
}
...
uint8_t JA_PN532::readGPIO(void)
{
  ...
  _interface->readResponse(pn532_packetbuffer, sizeof(pn532_packetbuffer));
...
bool JA_PN532::setPassiveActivationRetries(uint8_t maxRetries)
{
  ...
  if (_interface->writeCommand(pn532_packetbuffer, 5))
    return 0x0;  // no ACK
...
bool JA_PN532::readPassiveTargetID(uint8_t cardbaudrate, uint8_t *uid, uint8_t *uidLength, uint16_t timeout)
{
  ...
  // read data packet
  if (_interface->readResponse(pn532_packetbuffer, sizeof(pn532_packetbuffer), timeout) < 0) {
    return 0x0;
  }
...
uint8_t JA_PN532::mifareclassic_AuthenticateBlock (uint8_t *uid, uint8_t uidLen, uint32_t blockNumber, uint8_t keyNumber, uint8_t *keyData)
{
  ...
  if (_interface->writeCommand(pn532_packetbuffer, 10 + _uidLen))
    return 0;
...
uint8_t JA_PN532::mifareclassic_WriteDataBlock (uint8_t blockNumber, uint8_t *data)
{
  ...
  /* Send the command */
  if (_interface->writeCommand(pn532_packetbuffer, 20)) {
    return 0;
  }
...
uint8_t JA_PN532::mifareultralight_WritePage (uint8_t page, uint8_t *buffer)
{
  ...
  /* Send the command */
  if (_interface->writeCommand(pn532_packetbuffer, 8)) {
    return 0;
  }
...
bool JA_PN532::inDataExchange(uint8_t *send, uint8_t sendLength, uint8_t *response, uint8_t *responseLength)
{
  ...
  if (_interface->writeCommand(pn532_packetbuffer, 2, send, sendLength)) {
    return false;
  }

  int16_t status = _interface->readResponse(response, *responseLength, 1000);
  if (status < 0) {
    return false;
  }
...
bool JA_PN532::inListPassiveTarget()
{
  ...
  int16_t status = _interface->readResponse(pn532_packetbuffer, sizeof(pn532_packetbuffer), 30000);
  if (status < 0) {
    return false;
  }
...
int8_t JA_PN532::tgInitAsTarget(const uint8_t* command, const uint8_t len, const uint16_t timeout){
  ...
  int8_t status = _interface->writeCommand(command, len);
  if (status < 0) {
    return -1;
  }

  status = _interface->readResponse(pn532_packetbuffer, sizeof(pn532_packetbuffer), timeout);
  if (status > 0) {
    return 1;
...
int16_t JA_PN532::tgGetData(uint8_t *buf, uint8_t len)
{
  ...
  if (_interface->writeCommand(buf, 1)) {
    return -1;
  }

  int16_t status = _interface->readResponse(buf, len, 3000);
  if (0 >= status) {
    return status;
  }
...
bool JA_PN532::tgSetData(const uint8_t *header, uint8_t hlen, const uint8_t *body, uint8_t blen)
{
  if (hlen > (sizeof(pn532_packetbuffer) - 1)) {
    ...
    if (_interface->writeCommand(pn532_packetbuffer, 1, header, hlen)) {
      return false;
    }
  } else {
    ...
    if (_interface->writeCommand(pn532_packetbuffer, hlen + 1, body, blen)) {
      return false;
    }
  }

  if (0 > _interface->readResponse(pn532_packetbuffer, sizeof(pn532_packetbuffer), 3000)) {
    return false;
  }
...
int16_t JA_PN532::inRelease(const uint8_t relevantTarget){
  ...
  if (_interface->writeCommand(pn532_packetbuffer, 2)) {
    return 0;
  }

  // read data packet
  return _interface->readResponse(pn532_packetbuffer, sizeof(pn532_packetbuffer));
}
...
int8_t JA_PN532::felica_Polling(uint16_t systemCode, uint8_t requestCode, uint8_t * idm, uint8_t * pmm, uint16_t *systemCodeResponse, uint16_t timeout)
{
  ...
  int16_t status = _interface->readResponse(pn532_packetbuffer, 22, timeout);
  if (status < 0) {
...
int8_t JA_PN532::felica_SendCommand (const uint8_t *command, uint8_t commandlength, uint8_t *response, uint8_t *responseLength)
{
  ...
  if (_interface->writeCommand(pn532_packetbuffer, 3, command, commandlength)) {
    DMSG("Could not send FeliCa command\n");
    return -2;
  }

  // Wait card response
  int16_t status = _interface->readResponse(pn532_packetbuffer, sizeof(pn532_packetbuffer), 200);
  if (status < 0) {
    DMSG("Could not receive response\n");
    return -3;
  }
...
int8_t JA_PN532::felica_Release()
{
  ...
  // Wait card response
  int16_t frameLength = _interface->readResponse(pn532_packetbuffer, sizeof(pn532_packetbuffer), 1000);
  if (frameLength < 0) {
    DMSG("Could not receive response\n");
    return -2;
  }
